...
printWolfram (Args r s l w m) (wolfram : rest) =
    putStr (showStateList (reverse (take (w `div` 2) (leftList wolfram)))) >>
    putStrLn (showStateList (take ((w `div` 2) + (w `mod` 2)) (rightList
    wolfram))) >>
    printWolfram (Args r s (l - 1) w m) rest
